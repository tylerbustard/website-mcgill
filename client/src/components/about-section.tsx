...
export default function EducationSection() {
  ...
  const headerAnimation = useScrollAnimation({ threshold: 0.25, triggerOnce: true, delay: isMobile ? 0 : 80 });
  const heroCardAnimation = useScrollAnimation({ threshold: 0.1, rootMargin: '0px 0px 20% 0px', triggerOnce: true, delay: isMobile ? 0 : 80 });
  const { ref: achievementsRef, visibleItems } = useStaggeredScrollAnimation(2, { threshold: 0.1, triggerOnce: true, delay: isMobile ? 0 : 100 });
  const { ref: mcgillAchievementsRef, visibleItems: mcgillAchievementsItems } = useStaggeredScrollAnimation(2, { threshold: 0.1, triggerOnce: true, delay: isMobile ? 0 : 100 });
  const { ref: courseRef, visibleItems: courseVisibleItems } = useStaggeredScrollAnimation(6, { threshold: 0.1, triggerOnce: true, delay: isMobile ? 0 : 120 });
  const { ref: unbCourseRef, visibleItems: unbCourseVisibleItems } = useStaggeredScrollAnimation(12, { threshold: 0.1, triggerOnce: true, delay: isMobile ? 0 : 120 });
